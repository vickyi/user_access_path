...
yarn jar ./pathlist-1.0.jar com.imeijia.bi.path.PathComputeDriver $dt fct_path_compute_input_6levels test/path_list_offline_6levels 4
if test $? -ne 0
then
exit 11
fi
...
## 删除数据,否则数据翻倍了
hadoop fs -rm -r /user/hive/warehouse/dw.db/fct_path_list_off_6levels/date=$dt


loadStart=$(date +%s)
## load mapreduce 的输出结果至hive 表：fct_path_list_off_6levels
echo "=====>> load_to_hive 开始，将 hdfs 文件load至 dw.fct_path_list_off_6levels ......."
hive -d date=$dt -f ./script/load_data_to_6levels.sql
if test $? -ne 0
then
exit 11
fi
